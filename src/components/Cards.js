                ...
                data.map((e, i) => {
                    return (
                        <Link to={`/details/${e.id}`}>
                            <div key={i} className='card text-lg font-medium shadow-lg p-2 hover:-translate-y-3 cursor-pointer mt-6 transition-all duration-500'>
                                <img className='h-60 md:h-72' src={e.image} alt='Movie card' />
                                ...
                                <h1><span className='text-blue-500 mr-1'>Year:</span>{e.year}</h1>
                            </div>
                        </Link>
                    )
                })
